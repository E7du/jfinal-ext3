...
public class ReceiveResolverFactory {
    ...
    public ReceiveResolver createReceiveResolver(Integer messageType) {
        LOG.debug(" receive messageType " + messageType);
    ...
    private synchronized void init() {
        ...
        try {
            ...
            LOG.error("load ReceiveResolver error, it is defined in file" + resoruceLocation,e);
        }
        LOG.debug("resolvers in  " + typeFilter + " :" + receiveResolverMap);
    }
        ...
        for (String key : JmsConfig.keys()) {
            if (key.startsWith(typeFilter) && !key.endsWith(RESOLVER_SUFFIX)) {
                ...
                try {
                    ...
                    LOG.error("cant create " + messageResolver,e);
                }
            }
        }
    }
